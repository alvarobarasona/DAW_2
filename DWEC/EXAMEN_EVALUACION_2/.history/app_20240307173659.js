...
const path = require("path");
const dataPath = path.join(__dirname, "data", "data.json");
const moviesPath = path.join(__dirname, "public", "html", "movies.html");
const fs = require("fs").promises;
...
app.post("/loginuser", async (req, res) => {
    ...
    try {
        ...
        const jsonDataArray = jsonData.users;
        jsonDataArray.forEach((user) => {
            if(user.user.name === userData.userName && user.user.password === userData.password) {
                console.log("pula");
                res.send(moviesPath);
            }
        });
