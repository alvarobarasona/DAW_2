    ...
    */
    ...
    function logoutFunction($query_consume_token, $button_name, $remember_cookie_name, $redirect_path) {
        if(isPostMethod() && buttonExists($button_name)) {
            session_destroy();
            if(isset($_COOKIE)) {
                $db->ejecuta($query_consume_token);
...
/*
    function userLogin2(&$errors_array, $key_error, $user_query, $name_input, $password_input, $password_column, $redirect_path, $session_name, $remembre_input = null, $token_length = null, $token_expiration = null, $insert_token_query) {
        if(empty($errors_array)) {
            $user_name = $_POST[$name_input];
            $user_password = $_POST[$password_input];
            if(userExists($user_query, $user_name, $user_password, $password_column)) {
